...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const icon = (type: ToastType) =>
    type === "success" ? <CheckCircle2 size={16} className="text-[var(--mint)] shrink-0" /> : type === "error" ? <AlertCircle size={16} className="text-[var(--danger)] shrink-0" /> : <Info size={16} className="text-[#4EA8DE] shrink-0" />;
