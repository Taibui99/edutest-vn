  ...
  return (
    <div className="p-4 lg:p-8 max-w-3xl mx-auto">
      <h1 className="text-xl font-black text-[var(--text-primary)] mb-6 flex items-center gap-2">
        <Settings size={20} className="text-[#6C4CF1]" /> Cài đặt hệ thống
      </h1>
      ...
      <div className="flex gap-1.5 mb-5 flex-wrap">
        {TABS.map((t) => (
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${tab === t.key ? "bg-[#6C4CF1] text-white" : "bg-[var(--surface-card)] border border-[var(--surface-border)] text-[var(--text-secondary)] hover:border-[#6C4CF1]/40"}`}
          >
            {t.icon} {t.label}
          </button>
        ))}
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                {f.type === "toggle" ? (
                  <button
                    ...
                    disabled={saving}
                    className={`relative w-11 h-6 rounded-full transition-colors shrink-0 disabled:opacity-50 ${boolValue(f.key) ? "bg-[#6C4CF1]" : "bg-[var(--gray-200)]"}`}
                    aria-label={f.label}
                  >
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <input
                      ...
                      className="flex-1 rounded-lg border border-[var(--surface-border)] px-3 py-2 text-sm focus:border-[#6C4CF1] focus:outline-none"
                    />
                    <button
                      onClick={() => save(f.key, settings[f.key] ?? "")}
                      disabled={saving}
                      className="rounded-lg bg-[#6C4CF1] px-4 py-2 text-xs font-bold text-white hover:opacity-90 disabled:opacity-50"
                    >
                      Lưu
                    </button>
                  </div>
