...
export default async function ThiPage({ params, searchParams }: { params: Promise<{ code: string }>; searchParams: Promise<{ preview?: string }> }) {
  ...
  if (!session?.user) {
    ...
    return <ExamTakingClientV2 backHref="/vao-thi" exam={{ id: exam.id, title: exam.title, subject: exam.subject, durationMinutes: exam.durationMinutes, joinCode: exam.joinCode, isGuest: true, participantName: guest.name, participantClass: guest.className, shuffleQuestions: exam.shuffleQuestions, shuffleAnswers: exam.shuffleAnswers, questions: mapQuestions() }} />;
  }
  ...
  if (attemptsUsed >= exam.maxAttempts) return <div className="min-h-screen grid place-items-center bg-[#F8FAFC] p-4"><div className="rounded-2xl bg-white p-8 text-center"><Trophy className="mx-auto mb-4 text-[#16A34A]"/><p className="text-sm font-semibold">Bạn đã hết số lần làm bài</p>{latestSubmission && <p className="mt-2 text-3xl font-black text-[#16A34A]">{latestSubmission.score}/10</p>}<p className="mt-2 text-xs text-[#64748B]">Đã làm {attemptsUsed}/{exam.maxAttempts} lượt</p><Link href="/bang-dieu-khien" className="mt-5 inline-flex items-center gap-2 rounded-lg bg-[#2563EB] px-5 py-2.5 text-sm font-semibold text-white"><ArrowLeft size={14}/> Về trang chủ</Link></div></div>;
  return <ExamTakingClientV2 backHref="/bang-dieu-khien/de-thi" exam={{ id: exam.id, title: exam.title, subject: exam.subject, durationMinutes: exam.durationMinutes, joinCode: exam.joinCode, isGuest: false, shuffleQuestions: exam.shuffleQuestions, shuffleAnswers: exam.shuffleAnswers, questions: mapQuestions() }} />;
}
