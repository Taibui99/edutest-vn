...
export default async function ThiPage({ params, searchParams }: { params: Promise<{ code: string }>; searchParams: Promise<{ preview?: string }> }) {
  ...
  if (!session?.user) {
    ...
    if (guest.submittedAt) {
      ...
      if (sub) return <div className="min-h-screen grid place-items-center bg-[#F8FAFC] p-4"><div className="rounded-2xl bg-white p-8 text-center"><Trophy className="mx-auto mb-4 text-[#16A34A]"/><p className="text-sm font-semibold">Đã nộp bài</p><p className="mt-2 text-3xl font-black text-[#16A34A]">{sub.score}/10</p><p className="mt-1 text-sm text-[#64748B]">{guest.name} · {guest.className}</p></div></div>;
    }
