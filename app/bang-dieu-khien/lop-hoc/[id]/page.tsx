...
export default function ClassroomDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto animate-fade-in">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-[var(--primary)] to-[#9B7FF7] p-5 mb-6 relative overflow-hidden">
      ...
      <div className="flex gap-1 p-1 bg-[var(--gray-100)] rounded-xl mb-5 w-fit">
        {(["members", "exams"] as Tab[]).map((t) => (
          <button key={t} onClick={() => setTab(t)} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${tab === t ? "bg-white text-[var(--text-primary)] shadow-sm dark:bg-[#2B2358]" : "text-[var(--text-muted)] hover:text-[var(--text-secondary)]"}`}>
