      ...
      <div className="flex gap-1.5 mb-6 rounded-xl bg-[var(--gray-100)] p-1 w-fit">
        {([["mine", "Kho của tôi"], ["exams", "Trong đề thi"]] as const).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`rounded-lg px-4 py-2 text-sm font-bold transition ${tab === key ? "bg-white text-[var(--primary)] shadow-sm dark:bg-[#134E4A] dark:text-white" : "text-[var(--text-muted)]"}`}
          >
            {label}
          </button>
        ))}
