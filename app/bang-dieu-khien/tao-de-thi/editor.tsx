  ...
  return (
    <div className="min-h-full bg-[var(--surface-page)] px-4 py-5 lg:px-7 lg:py-7">
      <div className="mx-auto max-w-7xl">
        ...
        {error && <div className="mb-5 rounded-xl border border-[var(--danger-light)] bg-[var(--danger-light)] p-3 text-sm text-[var(--danger)]">{error}</div>}
            ...
            <div className="flex flex-col gap-4 p-5">
              {questions.map((q, i) => (
                <div key={i} className="rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-page)] p-4">
                  ...
                  {q.type === "mcq" && <div className="mt-3 grid gap-2 md:grid-cols-2">{q.options.map((opt, oi) => { const letter=String.fromCharCode(65+oi); const selected=q.answer===letter; return <div key={letter} className={cn("flex items-center gap-2 rounded-xl border p-2.5", selected ? "border-[var(--success)] bg-[var(--success-light)]" : "border-[var(--surface-border)] bg-[var(--surface-card)]")}><button onClick={()=>updateQuestion(i,{answer:letter})} className={cn("grid h-7 w-7 place-items-center rounded-full text-xs font-black",selected?"bg-[var(--success)] text-white":"bg-[var(--gray-100)] text-[var(--text-secondary)]")}>{letter}</button><input value={opt} onChange={(e)=>{const options=[...q.options]; options[oi]=e.target.value; updateQuestion(i,{options});}} placeholder={`Đáp án ${letter}`} className="min-w-0 flex-1 bg-transparent text-sm outline-none"/></div>;})}</div>}

                  {q.type === "true_false" && <div className="mt-3 flex flex-col gap-2">{(q.grading?.statements || [{text:"",answer:true}]).map((s,si)=><div key={si} className="flex gap-2 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] p-2.5"><input value={s.text} onChange={(e)=>{const statements=[...(q.grading?.statements||[])]; statements[si]={...statements[si],text:e.target.value}; updateQuestion(i,{grading:{...q.grading,statements}})}} placeholder={`Mệnh đề ${si+1}`} className="min-w-0 flex-1 bg-transparent text-sm outline-none"/><select value={String(s.answer)} onChange={(e)=>{const statements=[...(q.grading?.statements||[])]; statements[si]={...statements[si],answer:e.target.value==="true"}; updateQuestion(i,{grading:{...q.grading,statements}})}} className="rounded-lg border border-[var(--surface-border)] bg-[var(--surface-card)] px-2 text-sm text-[var(--text-primary)]"><option value="true">Đúng</option><option value="false">Sai</option></select></div>)}<Button size="sm" variant="outline" onClick={()=>updateQuestion(i,{grading:{...q.grading,statements:[...(q.grading?.statements||[]),{text:"",answer:true}]}})}>+ Thêm mệnh đề</Button></div>}
...
<aside className="flex flex-col gap-5 xl:sticky xl:top-5 xl:self-start">
      ...
      {preview && <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/50 p-4 backdrop-blur-sm"><div className="mx-auto max-w-4xl rounded-2xl bg-[var(--surface-card)] p-6 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div><p className="text-xs text-[var(--text-secondary)]">Xem trước</p><h2 className="text-xl font-black">{title || "Đề thi mới"}</h2></div><button onClick={()=>setPreview(false)} className="grid h-9 w-9 place-items-center rounded-lg bg-[var(--gray-100)]"><X size={17}/></button></div><div className="flex flex-col gap-4">{questions.map((q,i)=><div key={i} className="rounded-xl border p-4"><p className="mb-3 font-semibold">Câu {i+1}. {q.question}</p>{q.type==="mcq"&&q.options.map((o,oi)=><div key={oi} className="mb-1 rounded-lg border p-2 text-sm">{String.fromCharCode(65+oi)}. {o}</div>)}{q.type==="true_false"&&(q.grading?.statements||[]).map((s,si)=><div key={si} className="mb-1 flex justify-between rounded-lg border p-2 text-sm"><span>{String.fromCharCode(97+si)}) {s.text}</span><strong>{s.answer?"Đúng":"Sai"}</strong></div>)}{q.type==="short_answer"&&<div className="rounded-lg border bg-[var(--gray-100)] p-3 text-sm">Ô trả lời ngắn</div>}{q.type==="essay"&&<div className="min-h-28 rounded-lg border bg-[var(--gray-100)] p-3 text-sm">Ô trả lời tự luận</div>}</div>)}</div></div></div>}
    </div>
  );
}
