...
- [x] **Nhỏ-3 Đổi chế độ có confirm + loading** (commits `9cc071d`, `3c3b8ce`) — component `ModeSwitchButton` dùng chung (dashboard ModeSwitcher + admin sidebar): click → ConfirmDialog hỏi "Chuyển sang chế độ X?" → xác nhận → **loading screen toàn trang** (brand logo + thanh indeterminate, `animate-indeterminate` thêm globals.css), hủy thì đóng dialog — thay form submit im lặng; verified live: dialog hiện/đóng/hủy, loading screen overlay z-9999 + progress bar, mode đổi thành công
