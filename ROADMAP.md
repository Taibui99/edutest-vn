...
- [x] **Nhỏ-3 Đổi chế độ có confirm + loading** (commits `9cc071d`, `3c3b8ce`, `b055687`, `997885b`) — component `ModeSwitchButton` dùng chung (dashboard ModeSwitcher + admin sidebar + sidebar chính "Đổi chế độ"): click → ConfirmDialog hỏi "Chuyển sang chế độ X?" → xác nhận → **loading screen toàn trang** (brand logo + thanh indeterminate, `animate-indeterminate` thêm globals.css, nền blur `backdrop-blur-md`), hủy thì đóng dialog — thay form submit im lặng và Link no-op; verified live: dialog hiện/đóng/hủy, loading screen overlay z-9999 + progress bar + blur, mode đổi thành công, nút sidebar chính click mở dialog đúng
